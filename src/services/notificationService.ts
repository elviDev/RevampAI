...
class NotificationService {
  ...
  private async registerTokenWithBackend(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL || 'http://10.0.2.2:3001/api/v1'}/notifications/register`, {
        method: 'POST',
        headers: {
  ...
  async sendChannelNotification(options: {
    ...
    try {
      const token = await tokenManager.getCurrentToken();
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL || 'http://10.0.2.2:3001/api/v1'}/notifications/channel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  async updateNotificationPreferences(preferences: {
    ...
    try {
      const token = await tokenManager.getCurrentToken();
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL || 'http://10.0.2.2:3001/api/v1'}/notifications/preferences`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
